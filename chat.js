...
import { 
    ...
    orderBy, getDocs, getDoc, doc, updateDoc, deleteDoc, limit 
...
async function loadUserProfile() {
    ...
    if(userDoc.exists()) {
        ...
        if(data.avatar) {
            document.getElementById('settings-avatar-preview').innerHTML = `<img src="${data.avatar}">`
        }
    }
}
...
function loadMyChats() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach(async (chatDoc) => {
            ...
            div.className = `chat-item ${currentChatId === chatDoc.id ? 'active' : ''}`
            ...
            `
            div.onclick = () => openChat(chatDoc.id, otherUserId, otherUsername, otherAvatar)
...
function renderMessage(docSnap) {
    ...
    let content = ''
    if(msg.imageUrl) content += `<img src="${msg.imageUrl}" class="msg-image" onclick="window.viewImage('${msg.imageUrl}')">`
...
messageForm.addEventListener('submit', async (e) => {
    ...
    // Edit Mode
    if(editingMessageId) {
        await updateDoc(doc(db, 'messages', editingMessageId), { text, isEdited: true })
        cancelEdit()
        return
    }
    ...
    let imageUrl = null
    if(file) {
        const refStorage = ref(storage, `chat_${currentChatId}/${Date.now()}_${file.name}`)
...
// Settings
document.getElementById('open-settings-btn').onclick = () => window.openModal('modal-settings')
...
// Create Chat & Search
document.getElementById('fab-create-chat').onclick = () => window.openModal('modal-new-chat')

const searchInput = document.getElementById('user-search-input')
let searchTimeout = null // Переменная для таймера

searchInput.addEventListener('input', e => {
	const val = e.target.value.trim()
	const resultsContainer = document.getElementById('search-results')

	// Сброс результатов если пусто
	if (val.length === 0) {
	...
	// Ставим новый таймер (задержка 300мс)
	searchTimeout = setTimeout(async () => {
		// Показываем "Загрузку" пока ищем
		resultsContainer.innerHTML = `<div style="text-align:center; padding: 20px; color: var(--text-sec);"><div class="spinner" style="width:20px; height:20px; margin:0 auto;"></div></div>`
		...
		try {
			...
			if (snap.empty) {
				// Красивая заглушка "Не найдено"
				resultsContainer.innerHTML = `
                    <div class="search-empty">
                        <i class="ri-ghost-smile-line"></i>
                        <p>Пользователь не найден</p>
                        <span style="font-size: 0.8rem; color: var(--text-sec);">Проверьте правильность написания</span>
                    </div>
                `
				return
			}
			...
			snap.forEach(docSnap => {
				...
				const avatarHtml = user.avatar
					? `<img src="${user.avatar}" class="search-avatar-img">`
					: `<div class="search-avatar-placeholder">${user.username[0].toUpperCase()}</div>`
				...
				div.innerHTML = `
                    <div class="search-item-left">
                        ...
                        <div class="search-info">
                            ...
                            <span class="search-email" style="font-size: 0.75rem; opacity: 0.6;">Нажмите, чтобы написать</span>
                        </div>
                    </div>
                    <button class="btn-icon-small"><i class="ri-message-3-line"></i></button>
                `
			...
			resultsContainer.innerHTML = `<div style="text-align:center; color:#ff4d4d;">Ошибка поиска</div>`
		}
...
document.getElementById('ctx-delete').onclick = async () => {
    if(confirm('Удалить?')) await deleteDoc(doc(db, 'messages', contextMenuTargetId))
}
document.getElementById('ctx-pin').onclick = async () => {
    await updateDoc(doc(db, 'chats', currentChatId), { pinnedMessageId: contextMenuTargetId })
}
...
document.getElementById('close-viewer').onclick = () => document.getElementById('image-viewer').style.display = 'none'
